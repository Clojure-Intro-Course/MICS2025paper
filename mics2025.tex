...
Clojure is a hosted language, running on the Java Virtual Machine (JVM). As a result, it uses many of the features that the JVM provides,
	 but most importantly here is that it uses the Java Exception system for handling errors. When an exception is thrown in Clojure,
	  it is a Java exception, because Clojure code compiles to JVM bytecode.
   ...
   % \item \textbf{What are Exceptions} \\
	...
	exceptions typically occur when something goes wrong, such as dividing by zero or passing an inappropriate argument to a function.
	...
	Error messages are generated when an exception occurs. They provide information about the type of error as well as where it happens. Here is an example:
	\begin{figure}[h]
		\centering
	...
	\tkcomment{make it automatically wrap so it is not too wide}
	\begin{verbatim}
		#error {
 ...
 [[clojure.lang.RT seqFrom "RT.java" 557]
  ...
  [clojure.main main "main.java" 40]]}
	...
	\tkcomment{again the same problem with having to manually wrap text}
	\begin{verbatim}
		user=> (clojure.main/ex-triage(Throwable->map *e))
		#:clojure.error{:class java.lang.IllegalArgumentException,
		 :line 1, :cause "Don't know how to create ISeq from:
		  java.lang.Long", :symbol user/eval1, :phase :execution}
	\end{verbatim}
