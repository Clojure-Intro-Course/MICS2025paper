...
The flexible nature of spec allows programmers to choose what parameters to check and, to a degree, how the failure would manifest itself. 
\emcomment{and, to a degree, what information will be communicated about why the arguments failed to satisfy spec}.

\jscomment{Wording in the last sentence is a little bit cryptic, in my opinion. I'm not sure what "how the failure would manifest itself" refers to in relation to Clojure specs, especially if it's not clear that we're talking about spec failures or errors produced by them. (This is expanded on slightly later)}
...
\jscomment{"Failed to convert to each other?" I thought casting is strictly from one type to another. Possibly just iffy wording.}

\jscomment{I'd say something more like "...With more detailed information (at our disposal) provided by spec, we can make error messages that are more understandable ..."}
...
\emcomment{Clarify that this spec not the only way to generate error messages}
...
\emcomment{It's a temporary interpreter function, not a file - but also maybe just say ``And removing irrelevant internal infomration?". I also wouldn't include the stack trace in the examples.}
...
By employing specs on core Clojure functions, it is possible to explicitly determine the correct form of a function call and how the function was used incorrectly. Using spec, we know that the \verb|filter| function must take a predicate and, optionally, a collection, therefore expecting one or two arguments. With Babel, the above error message is reformatted as follows:\emcomment{rewritten, rather than reformatted? Or replaced by ...}
...
Our project presently includes specs for many functions in the \verb|clojure.core| library 
\emcomment{predefined Clojure functions?}
and rewrites a majority of potential Clojure errors that cannot employ spec (such as divisions by zero, unmatched delimiters, etc). However, \emcomment{I wouldn't use "however"} our research at this stage of the project largely concerns \emcomment{focusing on?} the potential of third party tools to display modified error messages in an interactive viewer that will allow beginner programmers to explore them more dynamically and familiarize themselves with how Clojure itself handles error data.
\emcomment{Perhaps give an example of what this means?}
